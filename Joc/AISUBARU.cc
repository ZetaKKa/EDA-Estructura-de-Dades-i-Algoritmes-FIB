   ...
   */
    ...
    void llegir_mapa(vector<vector<bool>>& V) {
        for (int i = 0; i < 60; ++i) {
            for (int j = 0; j < 60; ++j) {
                if (not V[i][j]) {
                    if (cell(Pos(i,j)).type == Waste or ((cell(Pos(i,j)).id != -1 and unit(cell(Pos(i,j)).id).type == Dead))) V[i][j] = true;
                }
            }
        }
    }
    */
    ...
    void move_alives(){
        ...
        for (int i = 0; i < n; ++i){
            ...
            vector <vector <bool> > V(60,vector<bool>(60,false));
            //llegir_mapa(V);
            ...
            Q.push(make_pair(p_ini,d_ini));
            
            //      primer   zombie      trobat
            ...
            */
                ...
                while (not Q.empty() and not Z and not found and not enemic) {
                    Pos p_act = Q.front().first;
                    Dir d_act = Q.front().second;
                    Q.pop();
